function staba_detector(filename)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Description: The function processes a *.mat data file (name: "filename") provided by the
%              user to extract the onset and offset time of HFO events in each channel. It
%              is assumed that the file contains a multichannel time series (double) with
%              time labels. The HFOs are determined by using the Staba algorithm (Staba et
%              al., J. Neurophysiol., 2002). The onset and offset time of each HFO are
%              returned in 2-D MATLAB arrays (one array per sequence) and stored in a
%              *.mat file in the current folder.
%
%-----------------------------------------------------------------------------------------
%
...
mindis = round(0.010*FS); maxdur = round(0.150*FS);
npks   = 6;
%-----------------------------------------------------------------------------------------
...
while (st_pnt<end_pnt)
    ...
    if (sum(pntval0)>0.4*WIN && flg)
        ...
        %   f) condition d) and e) must hold less than "maxdur" samples.
        %
        ...
        for W=1:ncol
            ...
            if (~isempty(locs))
                ...
                for r=1:size(cand,1)
                    ...
                    if (lcsv>=npks && (cand(r,2)-cand(r,1))<maxdur)
        ...
        % save the results
        HFO(end+1:end+size(final_HFO,1),:) = final_HFO;
...
if ~exist(sprintf('staba_times_%s',filename),'file')
    save(sprintf('staba_times_%s',filename),'-regexp','^HFO|^ncol|^FS');
else
    save(sprintf('staba_times_%s',filename),'-regexp','^HFO|^ncol|^FS','-append');
end
